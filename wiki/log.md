...
## 2026-06-08 — ingest paper
Ingestiti 2 PDF da `inbox/` in `raw/papers/`, rinominati per convenzione: `Doebereiner_2010_Model-and-Material-Compositional-Models.pdf` (tesi MA Sonology, L'Aja) e `Fletcher-Munson_1933_Loudness-Definition-Measurement-Calculation.pdf` (JASA 5(2):82-108, DOI verificato Crossref `10.1121/1.1915637`). Pagine wiki `wiki/sources/papers/`: doebereiner-2010 (CMSS: sintesi come atto compositivo, modello-di-suono = modello-di-composizione, intersezione sensibile/intelligibile, Koenig SSP/Brün SAWDUST; inquadra DSL Gamma e tensione modeling-the-receiver), fletcher-munson-1933 (paper fondativo loudness, origine curve isofoniche → radice storica della compensazione isofonica ISO 226 di Gamma). Entry `refs.bib` (Doebereiner2010, FletcherMunson1933), bibliography (✓) + index aggiornati. Escluso il terzo PDF (Castilho 2015, "Chaotic Systems as Compositional Algorithms"): report studentesco MUS-15 senza venue/DOI, non verificabile → scelta utente di non ingestire. `inbox/` svuotata. Merge branch `wiki/ingest-doebereiner-fletchermunson` → main.
